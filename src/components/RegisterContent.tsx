    ...
    <div className="w-full max-w-lg bg-[#1f1f24] rounded-2xl shadow-2xl p-10 border border-fuchsia-600/30">
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div className="relative">
          <input type="text" id="name" value={formData.name} onChange={handleChange} className="peer w-full px-4 pt-6 pb-2 text-sm bg-[#2a2a31] text-white rounded-md border border-transparent focus:outline-none focus:ring-2 focus:ring-fuchsia-500" placeholder=" " required />
          <label htmlFor="name" className="absolute left-4 top-2 text-xs text-gray-400 transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-500 peer-focus:top-2 peer-focus:text-xs peer-focus:text-fuchsia-400">Full Name</label>
        </div>
        <div className="relative">
        ...
        <button type="submit" className="w-full py-2 px-4 rounded-md text-white font-semibold bg-gradient-to-r from-fuchsia-600 to-violet-600 hover:from-fuchsia-500 hover:to-violet-500 transition-all duration-300 shadow-lg shadow-fuchsia-800/40">Sign Up</button>
      </form>
      {message && <p className="mt-4 text-center text-sm text-white">{message}</p>}
      <p className="text-sm text-gray-400 text-center mt-6">
        Already have an account?{" "}
        <a href="#/login" className="text-fuchsia-400 hover:underline">Log in</a>
      </p>
    </div>
  );
}
